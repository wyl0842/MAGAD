...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.img_dir = os.path.join(opt.outf, opt.name, opt.work_dir, 'images')
        self.tst_img_dir = os.path.join(opt.outf, opt.name, opt.test_dir, 'images')
    ...
    def print_result(self, performance, best):
    ...
    # def save_current_images(self, epoch, reals, fakes, fixed, erase):
    def save_current_images1(self, epoch, reals, fakes, erase):
        """ Save images for epoch i.
        ...
        Args:
            ...
            fixed ([FloatTensor]): Fixed Fake Image
        """
        train_out = torch.cat([erase, fakes, reals], dim=3)
        ...
        # # vutils.save_image(fixed, '%s/fixed_fakes_%03d.png' %(self.img_dir, epoch+1), normalize=True)
        # vutils.save_image(erase, '%s/erase_%03d.png' %(self.img_dir, epoch+1), normalize=True)
        vutils.save_image(train_out, '%s/train_out_%03d.png' %(self.img_dir, epoch+1), normalize=True)
