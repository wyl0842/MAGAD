...
class BaseModel():
    ...
    def set_input(self, input:torch.Tensor):
        ...
        with torch.no_grad():
            ...
            self.erase, self.patch_mask = patch_erase(self.input, patch_sz=(self.opt.inpainting_size, self.opt.inpainting_size))
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.dataloader['train'], leave=False, total=len(self.dataloader['train'])):
            ...
            #     self.visualizer.save_current_images1(self.epoch, reals, fakes, eraseinput)
            #     if self.opt.display:
    ...
    def test_3(self):
        ...
        if True:
            ...
            # pretrained_dict_d = torch.load(path_d)['state_dict']
            weight_dir = os.path.join(self.opt.outf, self.opt.name, self.opt.work_dir, 'weights')
            path = "{}/netG_101.pth".format(weight_dir)
            pretrained_dict = torch.load(path)['state_dict']
            
            path_d = "{}/netD_101.pth".format(weight_dir)
            pretrained_dict_d = torch.load(path_d)['state_dict']
            try:
                self.netg.load_state_dict(pretrained_dict)
                self.netd.load_state_dict(pretrained_dict_d)
    ...
    def test_res(self):
        ...
        img_list='/home/wangyl/data_wyl/Reconstruction/new_Dataset/test_baseline/1.abnormal'
        target_list=os.path.join(self.opt.outf, self.opt.name, self.opt.test_dir, 'recon_imgs101')        
    ...
    def train(self):
        ...
        # 保存模型参数设置
        message = "batchsize: {}\nisize: {}\nnz: {}\nname: {}\noutf: {}\nbeta1: {}\nw_adv: {}\nw_con: {}\nw_ssim: {}\nmem_size: {}\ninpainting_size: {}\n".format(
            self.opt.batchsize,self.opt.isize,self.opt.nz,self.opt.name,self.opt.outf, self.opt.beta1,self.opt.w_adv,self.opt.w_con,self.opt.w_ssim,self.opt.mem_size,self.opt.inpainting_size)
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                if self.opt.save_test_images :
                    ...
                    if not os.path.isdir(dst):
                        os.makedirs(dst)
                    real, fake, _, erase = self.get_current_images()
                    test_out = torch.cat([erase, fake, real], dim=3)
                    # vutils.save_image(real, '%s/real_%03d.eps' % (dst, i+1), normalize=True)
                    # vutils.save_image(fake, '%s/fake_%03d.eps' % (dst, i+1), normalize=True)
                    vutils.save_image(test_out, '%s/test_out_%03d.png' % (dst, i+1), normalize=True)
