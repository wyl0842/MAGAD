...
class NetG(nn.Module):
    ...
    def __init__(self, opt):
        ...
        layers = list(self.encoder1.main.children())
        self.features_map = nn.Sequential(*layers[:-1])
        self.mem_rep = MemModule(mem_dim=opt.mem_size, fea_dim=256, shrink_thres =0.0025)
    ...
    def forward(self, x):
        ...
        small_map = self.features_map(x)
        res_mem = self.mem_rep(small_map)
        f = res_mem['output']
        att = res_mem['att']
        gen_imag = self.decoder(f)
        #print(gen_imag.shape)
        latent_o = self.features_map(gen_imag)
        return gen_imag, small_map, latent_o
