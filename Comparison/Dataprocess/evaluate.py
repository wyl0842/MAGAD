...
def hot_map(result,tr_acc):
    ...
    np.savetxt(('class_precision.txt'), cls_pre, fmt="%f", delimiter="\n")
...
if __name__ == "__main__":
    ...
    gt_labels = np.loadtxt('gt_label.txt', dtype=float, delimiter='\n')
    an_scores = np.loadtxt('pred.txt', dtype=float, delimiter='\n')
    gt_class = np.loadtxt('gt_class.txt', dtype=float, delimiter='\n')
    ...
    # print(f1)
    print(auc_score)
    acc, pre, rec = ACC(gt_labels, an_scores, gt_class)
    print(acc)
    print(pre)
    print(rec)
