...
model = ['DSEBM', 'f-AnoGAN', 'AE', 'Ganomaly', 'MAGAD(ours)']
...
plt.plot(fpr, tpr, color=color[0], linestyle=':', lw=lw, label='{}(AUC = {:.3f})'.format(model[0], roc_auc))
...
plt.plot(fpr, tpr, color=color[1], linestyle=(0,(4,1.5,1,1.5,1,1.5)), lw=lw, label='{}(AUC = {:.3f})'.format(model[1], roc_auc))
# plt.plot([eer], [1-eer], marker='o', markersize=4, color="navy")
...
for method in methods:
    ...
    plt.plot(fpr, tpr, color=color[i],
         lw=lw, linestyle=linetype[i-2], label='{}(AUC = {:.3f})'.format(model[i], AUC)) ###假正率为横坐标，真正率为纵坐标做曲线
    # plt.plot([eer], [1-eer], marker='o', markersize=4, color="navy")
    i = i + 1
...
plt.xlabel('FPR')
plt.ylabel('TPR')
plt.xticks(fontsize=18)  # 调整x轴刻度字体大小为10
plt.yticks(fontsize=18)  # 调整y轴刻度字体大小为10
#plt.title('AUC_result')
plt.legend(loc="lower right", fontsize=18)
plt.grid()
# plt.savefig("AUC_result.png")
plt.savefig('plot20240407/AUC_result.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
plt.show()
