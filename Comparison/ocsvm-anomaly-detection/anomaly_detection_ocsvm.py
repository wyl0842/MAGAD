...
def parse_args():
    ...
    parser.add_argument('--data_path', default='/home/wangyl/data_wyl/MAGAD/Comparison/OCSVM/output_cae', type=str, help='path to dataset')
...
def traindata_gen(datapath, shape, batchsize=32):
    ...
    X = np.zeros((batchsize, shape))
    
    data = np.load(datapath)

    imagedata = data['ae_out']
...
def main():
    ...
    train_images, train_labels = load_data(os.path.join(data_path, 'output_traincae1.npz'))
    test_images, test_labels = load_data(os.path.join(data_path, 'output_testcae1.npz'))
    ...
    # train_datagen = traindata_gen(os.path.join(data_path, 'output_traincae.npz'), shape=1024, batchsize=8)
    ...
    for nu in nus:
        ...
        # clf.fit_generator(train_datagen)
        ...
        for j in range(test_rep_count):
            ...
            print(scores.shape)
            print(testy.shape)
            # dets = np.concatenate((scores.T, testy.T), axis=1)
            np.savetxt("testy_{}.txt".format(j), testy.T, fmt='%f', delimiter=',')
            np.savetxt("scores_{}.txt".format(j), scores.T, fmt='%f', delimiter=',')
