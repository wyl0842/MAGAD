...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        splits = ['train', 'val', 'test', 'test_baseline']
        drop_last_batch = {'train': True, 'val': True, 'test': False, 'test_baseline': True}
        shuffle = {'train': True, 'val': True, 'test': False, 'test_baseline': False}
        ...
        dataset = {x: CustomImageFolder(os.path.join(opt.dataroot, x),[transform]) for x in splits}
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataset[x],
                                                     batch_size=opt.batchsize,
                                                     shuffle=shuffle[x],
                                                     num_workers=int(opt.workers),
...
class CustomImageFolder(ImageFolder):
    ...
    def __getitem__(self, index1):
        if index1>=len(self.imgs):
            print(index1,len(self.imgs))
