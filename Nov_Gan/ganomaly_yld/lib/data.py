...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        dataset['test']=CustomImageFolder(os.path.join(opt.dataroot, 'test'),transform)
        test_size=opt.test_set_size
        #print(test_size)
        val_size = len(dataset['test']) - 1500
        val_dataset, test_dataset = torch.utils.data.random_split(dataset['test'], [1500, val_size])
